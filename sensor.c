 ...
 * Descripción: Ejecuta el sensor que lee datos desde un archivo y los envía por
 * un pipe.
 */
void EjecutarSensor(int argc, char *argv[]) {
  ...
  float value;
  ssize_t bytes_written; // representar el tamaño de un objeto en bytes o la
                         // cantidad de bytes leídos o escritos por funciones de
                         // entrada/salida (I/O)
  ...
  tiempo_espera =
      atoi(tiempo); // entra como cadena de caracteres y se necesita en entero
  ...
  while (fgets(line, sizeof(line), file)) { // leer línea por línea
    ...
    snprintf(buffer, sizeof(buffer), "%d:%f\n", sensor_type,
             value); // permite evitar desbordamientos de buffer al especificar
                     // el tamaño máximo del buffer donde se almacenará la
                     // cadena formateada
