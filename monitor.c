...
#define PH_FLAG "PH"
#define END_OF_DATA                                                            \
  -9999 // cuando se acaban sensores, el monitor tiene que también que acabar
...
/*
  ...
  Descripción: Inicializa los buffers y los semáforos. Ningún semáforo se puede
  compartir entre procesos
 */
void InicializarBuffers() {
  sem_init(
      &buffer_temp.empty, 0,
      BUFFER_SIZE); // Indica que todas las posiciones están vacías al inicio.
                    // Al final es el tamaño del semáforo. Se le hace un semwait
  sem_init(
      &buffer_temp.full, 0,
      0); // No hay posiciones llenas al inicio. Se le hace un semsignal/sempost
  sem_init(&buffer_temp.mutex, 0,
           1); // El buffer está disponible para acceso. Se le hace ambos para
               // asegurar exclusión mútua. Por eso empieza en 1
...
/*
  ...
  Descripción: Libera los recursos de los buffers y los semáforos. En mismo
  orden de creación
 */
void LiberarBuffers() {
...
/*
  ...
  Descripción: Lee datos desde un pipe y los coloca en los buffers
  correspondientes. Actúa como "productor", o sea que genera datos y los coloca
...
void *Recolector(void *arg) {
  ...
  while (read(fd, read_buffer, sizeof(read_buffer)) >
         0) { // leer datos desde un pipe, procesarlos y colocarlos en el buffer
              // adecuado (temperatura o pH), mientras haya datos
    if (read_buffer[strlen(read_buffer) - 1] == '\n') { // procesar cada línea
      read_buffer[strlen(read_buffer) - 1] =
          '\0'; // Eliminar el salto de línea, para que se vea bonito
    }

    char *token =
        strtok(read_buffer, ":"); // A partir de los : toma esa cadena de
                                  // caracteres y la coge como el token.
    sensor_type = atoi(token);
    token = strtok(
        NULL, ":"); // que se utiliza para dividir una cadena en una serie de
                    // tokens, delimitados por uno o más caracteres. Para
                    // continuar y en siguiente ejecución volver al inicio
    char *data =
        token; // para poder manipular y convertir ese valor a un formato
               // numérico adecuado para su posterior procesamiento

    if (sensor_type == 1 || sensor_type == 2) { // Temperatura o pH
      value = atof(data);
      // verificar, si es inválido (negativo, porque es el condición que se
      // comparte) sigue y no se rompe
      if (value < 0) {
      ...
      *in = (*in + 1) %
            BUFFER_SIZE; // una cajita y el espacio. Se asigna en el primer
                         // espacio vacío y se suma 1. Se hace ccircular y se
                         // asegura que meintras avance, siga dentro de los
                         // límites del buffer

      sem_post(mutex); // libera para darle paso al siguiente en la siguiente
                       // iteración.
      sem_post(full);
    } else {
  ...
  // Esperar 10 segundos y luego enviar mensaje de finalización en caso de que
  // ya no tenga mas sensores conectados
  sleep(10);

  // Colocar mensaje de finalización en cada buffer
  sem_wait(
      &buffer_temp
           .empty); // se inicializa con el tamaño del buffer (BUFFER_SIZE), y
                    // se decrementa cada vez que se produce un elemento (se
                    // coloca un dato en el buffer).
  sem_wait(&buffer_temp.mutex);
  buffer_temp.buf[buffer_temp.in] =
      END_OF_DATA; // marcador de finalización (-9999). el .buf es el buffer
                   // circular
  buffer_temp.in = (buffer_temp.in + 1) %
                   BUFFER_SIZE; // espera al menos una posión vacía del buffer,
                                // y como es circualr, sigue
  ...
  sensors_active = 0;
  unlink(pipe_name); // cerrar el pipe para que no se quede el monitor popr
                     // siempre esperando por datos que ya no entran
...
void *HiloTemp() {
  ...
  while (1) {
    sem_wait(&buffer_temp.full); // resta tamaño de lo que entra (región
                                 // crítica)
    sem_wait(&buffer_temp.mutex); // exclusión mutua

    temp = buffer_temp.buf[buffer_temp.out];
    buffer_temp.out =
        (buffer_temp.out + 1) % BUFFER_SIZE; // in era para recibir, entonces
                                             // out para escribir (sacarlo)
    ...
    if (temp >= 20.0 && temp <= 31.6) {
      strftime(timestamp, sizeof(timestamp), "%Y-%m-%d %H:%M:%S",
               tm_info); // formato fechas que toca hacer así
...
void EjecutarMonitor(int argc, char *argv[]) {
  ...
  if (argc != 9) {
    fprintf(
        stderr,
        "Modo de uso: %s -b buffer_size -t temp_file -h ph_file -p pipe_name\n",
        argv[0]);
    exit(1);
  }
  ...
  pthread_create(
      &recolector_thread, NULL, Recolector,
      pipe_name); // 1. id almacen del hilo, 2. objeto de atributos para crear
                  // el hilo pero ya se hizo, 3. rutina inicio o sea funicón a
                  // la que corresponde, y 4. apunta al único arg que se
                  // necesita para la rutina.
